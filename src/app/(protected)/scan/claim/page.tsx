  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (already) {
        ...
        if (creditsAwarded > 0) {
          setPill(`+${creditsAwarded} Credits • Gesamt: ${creditsTotal}`);
        } else {
          setPill(`Credits: ${creditsTotal}`);
        }
      }

      // Kurz stehen lassen, dann weiter
      setTimeout(() => {
        router.push("/meine-badges");
        // optional: refresh damit serverseitiger Header sofort neu rendert (hilft in PWA)
        router.refresh();
      }, 900);
    }
